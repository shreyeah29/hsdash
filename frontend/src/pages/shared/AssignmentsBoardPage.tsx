...
export type AssignmentsBoardMode = "admin" | "coordinator";
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="space-y-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          >
            {isAdmin ? "Studio command" : "Assignment control"}
          </p>
          <h1 className="mt-1 text-3xl font-semibold tracking-tight text-zinc-900">
            {isAdmin ? "Assign deliverables" : "Production routing"}
          </h1>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-600">
            {isAdmin
              ? "Same power as the coordinator board: pick an editor for each deliverable and their dashboard updates instantly with notifications."
          ...
          <Link
            to={calendarPath}
            className="skiper-link-accent mt-4 inline-flex text-sm font-medium text-violet-700"
          >
            Open shoot calendar →
          </Link>
        </div>
        ...
        <GlassPanel className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:gap-4" shine>
          ...
          <div
            className={cn(
              ...
              isAdmin ? "border-violet-200 bg-violet-50 text-violet-900" : "border-amber-200 bg-amber-50 text-amber-900",
            )}
          >
            <Users className={cn("h-4 w-4", isAdmin ? "text-violet-700" : "text-amber-700")} />
            <span>
      ...
      {!isLoading && filtered.length === 0 ? (
        <GlassPanel className="p-14 text-center shine">
          ...
          <p className="mt-2 text-sm text-zinc-600">
            No tasks match these filters — activate deliverables from a shoot on the{" "}
            <Link to={calendarPath} className="font-medium text-violet-700 underline">
              calendar
            </Link>
            .
          </p>
                        ...
                        <div className="flex flex-col gap-3">
                          <div className="flex flex-col gap-1.5">
                            ...
                            <Select
                              ...
                              onValueChange={(v) => assign.mutate({ id: t.id, assignedToId: v === "__none__" ? null : v })}
                            >

...
const COLUMNS: { key: TaskStatus; label: string; blurb: string; tint: string }[] = [
  { key: TaskStatus.PENDING, label: "Awaiting pickup", blurb: "Routing pending", tint: "from-violet-500/15 to-transparent" },
  {
  ...
  { key: TaskStatus.DELAYED, label: "Escalations", blurb: "Needs visibility", tint: "from-rose-500/18 to-transparent" },
  {
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="space-y-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-semibold tracking-tight text-white">Deliverables status</h1>
        ...
        <GlassPanel className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:gap-4" shine>
          ...
          <div className="flex items-center gap-2 rounded-xl border border-violet-400/20 bg-violet-500/10 px-3 py-2 text-xs text-violet-100/85">
            <Radar className="h-4 w-4 text-violet-300" />
            <span>
              <span className="font-semibold text-white">{filtered.length}</span> live signals
            </span>
          </div>
        </GlassPanel>
      </div>
      ...
      {error ? (
        <GlassPanel className="border-rose-500/25 bg-rose-950/20 p-6 text-sm text-rose-200">Failed to load deliverables.</GlassPanel>
      ) : null}

      {!isLoading && filtered.length === 0 ? (
        <GlassPanel className="p-14 text-center shine">
          <p className="text-sm font-medium text-white">No matching deliverables</p>
                  ...
                  <PriorityShowcaseCard
                    ...
                    metaRow={
                      <p className="text-[11px] text-zinc-500">
                        <span className="font-medium text-zinc-400">{t.assignedTeam.replaceAll("_", " ")}</span>
                        {" · "}
                        <span className="text-zinc-300">{t.assignedTo?.name ?? "Unassigned"}</span>
                        {" · "}
